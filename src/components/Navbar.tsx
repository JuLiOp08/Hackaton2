...
function Navbar() {
  return (
    <nav className="w-full bg-gray-100 shadow px-8 py-6 flex gap-8 items-center justify-center fixed top-0 left-0 z-50">
      ...
      <Link to="/get_students" className="px-3 py-1 rounded hover:bg-gray-300 font-semibold">Ver Estudiantes</Link>
      <Link to="/student/id" className="px-3 py-1 rounded hover:bg-gray-300 font-semibold">Buscar por ID</Link>
      <Link to="/student/delete" className="px-3 py-1 rounded hover:bg-gray-300 font-semibold">Eliminar Estudiante</Link>
    </nav>
  );
}
